...
"component 'test-fixture' do |pkg, settings, platform|
  ...
  describe '#add_actions' do
    ...
    it 'adds the correct trigger action to the component' do
      comp = Vanagon::Component::DSL.new('action-test', {}, dummy_platform_sysv)
      comp.add_install_triggers(['install', 'upgrade'], ['chkconfig --list', '/bin/true'], 'puppet-agent')
      comp.add_install_triggers('install', 'echo "hello, world"', 'puppet-agent')
    ...
    it 'fails with bad install trigger action' do
      comp = Vanagon::Component::DSL.new('action-test', {}, dummy_platform_sysv)
      expect { comp.add_install_triggers('foo', '/bin/true', 'puppet-agent') }.to raise_error(Vanagon::Error)
    end

    it 'fails with empty install trigger action' do
      comp = Vanagon::Component::DSL.new('action-test', {}, dummy_platform_sysv)
      expect { comp.add_install_triggers([], '/bin/true', 'puppet-agent') }.to raise_error(Vanagon::Error)
    end
    ...
    it 'adds debian activate triggers' do
      ...
      expect(comp._component.activate_triggers.first.interest_name).to eq ('puppet-agent-activate')
    end
